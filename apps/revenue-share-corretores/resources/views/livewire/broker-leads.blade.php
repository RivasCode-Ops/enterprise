<div>
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-600">Contatos recebidos pelos seus anúncios. Ao fechar negócio, registre a venda para gerar sua comissão e o repasse.</p>
    </div>
    ...
    @if (session('sale_registered'))
        <div class="mb-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800" role="alert">
            Venda registrada. Sua comissão foi lançada como pagamento <strong>pendente</strong>.
        </div>
    @endif

    @if ($leads->isEmpty())
        <div class="rounded-lg border border-dashed border-slate-300 bg-white p-12 text-center text-slate-600">
            Nenhum lead ainda. Divulgue o link público do imóvel para receber contatos.
        </div>
    @else
        <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tr>
                    ...
                    <th class="px-4 py-3 text-left font-semibold text-slate-700">Negócio</th>
                </tr>
                </thead>
                        ...
                        <td class="whitespace-nowrap px-4 py-3">
                            @if ($lead->sale)
                                ...
                                <p class="mt-1 text-xs text-slate-600">
                                    Comissão: R$ {{ number_format((float) $lead->sale->revenue_broker, 2, ',', '.') }}
                                </p>
                            @else
    ...
    @if ($showSaleModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative z-10 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                ...
                <p class="mt-1 text-sm text-slate-600">Sua comissão é calculada automaticamente: valor da venda × percentual ÷ 100.</p>
                ...
                <form wire:submit="saveSale" class="mt-4 space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700">Comissão (%)</label>
