<div class="space-y-4">
    <h2 class="text-lg font-semibold text-slate-900">
        {{ $propertyId ? 'Editar anúncio' : 'Novo anúncio' }}
    </h2>
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <div>
            ...
            <p class="mt-0.5 text-xs text-slate-500">Destaque diferenciais do imóvel: localização, metragem, condições de pagamento.</p>
        ...
        @if ($existingImages->isNotEmpty())
            <div>
                <p class="text-sm font-medium text-slate-700">Imagens do anúncio</p>
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700">
                {{ $propertyId ? 'Adicionar imagens' : 'Imagens' }}
            </label>
            ...
            <p class="mt-1 text-xs text-slate-500">Formatos: JPEG, PNG ou WebP. Até 5 MB por arquivo — imagens nítidas geram mais contatos.</p>
            @error('photos') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            @error('photos.*') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
        </div>

        <div wire:loading wire:target="photos" class="text-sm text-slate-500">Enviando imagens…</div>
